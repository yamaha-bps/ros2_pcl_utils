...
PclSegComponent::PclSegComponent(const rclcpp::NodeOptions &opts)
  ...
  RCLCPP_INFO(get_logger(), "Filtering classes [%s]",
...
void PclSegComponent::cb_pcl_(sensor_msgs::msg::PointCloud2::UniquePtr msg) {
  ...
  if (pImpl->frames.find(msg->header.frame_id) == pImpl->frames.end()) {
    if (!tf2_buf_->canTransform(pImpl->camera_frame, msg->header.frame_id,
                                msg->header.stamp)) {
      RCLCPP_WARN(get_logger(), "Can not transform from %s to %s",
    ...
    pImpl->frames[msg->header.frame_id] = Sophus::SE3f(
        Eigen::Quaternionf(tf.transform.rotation.w, tf.transform.rotation.x,
                           tf.transform.rotation.y, tf.transform.rotation.z),
        Eigen::Vector3f(tf.transform.translation.x, tf.transform.translation.y,
...
void PclSegComponent::work() {
  while (!canceled_) {
    ...
    if (it != pImpl->frames.end()) {
      ...
      filter(*pImpl->img_queue[idx], pImpl->calib, pImpl->classes_, P_CAM_LID,
             *pcl);
...
RCLCPP_COMPONENTS_REGISTER_NODE(cbr::PclSegComponent)
